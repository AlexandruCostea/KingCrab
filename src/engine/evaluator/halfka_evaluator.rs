...
use crate::engine::{board::board::Board, definitions::{NrOf, Piece, Side, SQUARE_BITBOARDS}, evaluator::evaluator::Evaluator};
...
impl HalfkaEvaluator {
    pub fn new(model_dir: &str) -> Result<Self, String> {
        ...
        let input_path = Path::new(model_dir).join("halfka_input_processor.onnx");
        ...
        for i in 0..8 {
            let bucket_path = Path::new(model_dir).join(format!("halfka_bucket_evaluator_{i}.onnx"));
        ...
        let own_embeddings_path = Path::new(model_dir).join("halfka_embeddings_own.npy");
        let opp_embeddings_path = Path::new(model_dir).join("halfka_embeddings_opp.npy");
    ...
    fn feature_index(piece_idx: usize, piece_sq: usize, king_sq: usize) -> Option<usize> {
        if piece_sq == king_sq {
            None
        } else {
    ...
    pub fn compute_halfka_indices(&self, board: &Board, side: Side) -> Vec<usize> {
        ...
        for i in 0..NrOf::SQUARES {
            ...
            let piece_char = piece.to_string().chars().next().unwrap_or(' ');
            let piece_idx = *self.piece_indices.get(&piece_char).unwrap_or(&0);
            ...
            let feature_idx = Self::feature_index(piece_idx, piece_square, king_sq);
    ...
    fn sum_embedding(&self, embedding: &Array2<f32>, indices: &[usize]) -> Array2<f32> {
...
impl Evaluator for HalfkaEvaluator {
    fn evaluate_board(&mut self, board: &Board) -> f32 {
        ...
        let new_own_set: HashSet<usize> = HashSet::from_iter(own_indices.iter().cloned());
        let new_opp_set: HashSet<usize> = HashSet::from_iter(opp_indices.iter().cloned());
        ...
        if self.cache.is_empty() {
            ...
            let added: Vec<usize> = new_own_set.difference(&self.cache.own_indices).cloned().collect();
            let removed: Vec<usize> = self.cache.own_indices.difference(&new_own_set).cloned().collect();
            ...
            let added: Vec<usize> = new_opp_set.difference(&self.cache.opp_indices).cloned().collect();
            let removed: Vec<usize> = self.cache.opp_indices.difference(&new_opp_set).cloned().collect();
        ...
        let value_own = Value::from_array(self.input_session.allocator(), &input_own).unwrap();
        let value_opp = Value::from_array(self.input_session.allocator(), &input_opp).unwrap();

        let outputs = self.input_session.run(vec![value_own, value_opp]).unwrap();
        ...
        let x_1024 = x_1024_dyn.view().to_owned().into_dimensionality::<ndarray::Ix2>()
            .expect("Expected (1, 1024) array");

        let avg_score = avg_score_dyn.view().to_owned().into_dimensionality::<ndarray::Ix2>()
        ...
        let x_val = Value::from_array(self.bucket_sessions[bucket_index].allocator(), 
                                                &input_x_val).unwrap();
        let avg_val = Value::from_array(self.bucket_sessions[bucket_index].allocator(),
                                                &input_avg_val).unwrap();

        let result = self.bucket_sessions[bucket_index].run(vec![x_val, avg_val]).unwrap();
